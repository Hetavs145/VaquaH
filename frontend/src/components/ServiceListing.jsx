...
const ServiceListing = ({ limit, showTitle = false }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        const fetchServices = async () => {
            try {
                ...
                setServices(limit ? fetchedServices.slice(0, limit) : fetchedServices);
            } catch (error) {
    ...
    return (
        <>
            {showTitle && <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">Our Services</h2>}
